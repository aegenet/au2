...
describe('code-mirror', () => {
  ...
  it('should render code with language', async () => {
    ...
    await renderInDOM(
      `<code-mirror code="${code}" language="javascript"></code-mirror>`,
      [CodeMirror],
      async result => {
  ...
  it('should render code modification', async () => {
    ...
    await renderInDOM(
      `<code-mirror autofocus.bind="true" code="${code}" language="javascript"></code-mirror>`,
      [CodeMirror],
      async result => {
        expect(result.textContent!.indexOf(code) !== -1).toBe(true);
  ...
  it('should render suggestions (none)', async () => {
    const code = "const sample = { code: 'oh' };";
    await renderInDOM(
      `<code-mirror autofocus.bind="true" code="${code}" suggestions.bind='[{ term: "<base-", values: ["<base-component>", "<base-page>"] }]' language="javascript"></code-mirror>`,
      [CodeMirror],
      async result => {
        expect(result.textContent!.indexOf(code) !== -1).toBe(true);
        const vm = getViewModel<CodeMirror>(result);
